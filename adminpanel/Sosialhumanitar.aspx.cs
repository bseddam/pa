...
public partial class adminpanel_Sosialhumanitar : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Page.IsPostBack == false)
        {
            ...
            LinkButton tb = cp.FindControl("lbsosialhum") as LinkButton;
    ...
    void regionihb()
    {
        DataTable dtiller = klas.getdatatable("Select ClassifID,ClassifName,ListID from List_classifications_SHablon where ListID=10 order by SortA_Z");
    ...
    {
        DataTable dtiller2 = klas.getdatatable("Select ClassifID,ClassifName,ListID from List_classifications_SHablon where ListID=10 order by SortA_Z");
    ...
    {
        DataRow drrayon = klas.GetDataRow("Select ClassifID,ClassifName,ListID from List_classifications_SHablon where ListID=10  and ClassifName=N'" + txtmeqsedredakte.Text + "'");

        if (ddlmeqsedredakte.SelectedValue != "0" && txtmeqsedredakte.Text != "" && drrayon == null)
        {
            SqlConnection baglanti = klas.baglan();
            SqlCommand cmd = new SqlCommand("Update List_classifications_SHablon Set ClassifName=@ClassifName Where ListID=10 and ClassifID=" + ddlmeqsedredakte.SelectedValue, baglanti);
            ...
            ddlmeqsedredakte.Items.Insert(0, new ListItem("Seçin", "0"));
            txtmeqsedredakte.Text = "";
            Response.Redirect("Sosialhumanitar.aspx");
        }
        else
        {
            if (ddlmeqsedredakte.SelectedValue == "0")
            {
                lblmeqsedredakte.Text = "Sosial və humanitar sahə seçməmisiniz";
            }
            if (txtmeqsedredakte.Text == "")
            {
                lblmeqsedredakte.Text = "Sosial və humanitar sahə boşluq ola bilməz";
            }
            if (drrayon != null)
            {
    ...
    protected void btnmeqsedelave_Click(object sender, EventArgs e)
    {
        ...
        {
            DataRow drrayon = klas.GetDataRow("Select ClassifID,ClassifName,ListID from List_classifications_SHablon where ListID=10 and ClassifName=N'" + txtmeqsedelave.Text + "'");
            if (drrayon == null && ddlmeqsedsort.SelectedValue != "-1")
            {
                if (ddlmeqsedsort.SelectedValue != "0")
                {
                    classifid = klas.GetDataRow("Select * from List_classifications_SHablon where ListID=10 and ClassifID='" + ddlmeqsedsort.SelectedValue + "'");
                    classifid1 = classifid["SortA_Z"].ToString();
                }
                else
                {
                    classifid = klas.GetDataRow("Select MIN(SortA_Z) as SortA_Z from List_classifications_SHablon Where ListID=10");
                ...
                DataTable sorthamisi = klas.getdatatable("Select * from List_classifications_SHablon Where ListID=10");
                for (int i = 0; i < sorthamisi.Rows.Count; i++)
                {
                    if (secilmiwsortid < int.Parse(sorthamisi.Rows[i]["SortA_Z"].ToString()))
                    {
                        ...
                        SqlConnection baglanti = klas.baglan();
                        SqlCommand cmd1 = new SqlCommand("Update List_classifications_SHablon Set SortA_Z=@SortA_Z Where ListID=10 and SortA_Z='" + a.ToString() + "' and ClassifID='" + classif.ToString() + "'", baglanti);
                ...
                cmd.ExecuteNonQuery();
                lblmeqsedelave.Text = "Sosial və humanitar sahə əlavə edildi";
        ...
        {
            lblmeqsedelave.Text = "Sosial və humanitar sahə adı boşluq ola bilməz";
            lblmeqsedelave.ForeColor = Color.Red;
        }
    }
    protected void btnmeqsedsil_Click(object sender, EventArgs e)
    {
        if (ddlmeqsedsil.SelectedValue != "0")
        {
            DataRow classifid;
            string classifid1;
            classifid = klas.GetDataRow("Select * from List_classifications_SHablon where ListID=10 and ClassifID='" + ddlmeqsedsil.SelectedValue + "'");
            classifid1 = classifid["SortA_Z"].ToString();
            int secilmiwsortid = int.Parse(classifid1.ToString());
            ...
            klas.cmd("Delete from List_classifications_SHablon  Where ListID=10 and ClassifID=" + ddlmeqsedsil.SelectedValue);
    ...
    protected void btnihbelave_Click(object sender, EventArgs e)
    {
        ...
        if (txtihbelave.Text != "" && ddlihbrayonelave.SelectedValue != "0")
        {
            ...
            if (drrayon == null && ddlmovqe.SelectedValue != "-1")
            {
                ...
                cmd.ExecuteNonQuery();
                lblihbelave.Text = "Sosial və humanitar sahə əlavə edildi";
        ...
        {
            lblihbelave.Text = "Sosial və humanitar sahə boşluq ola bilməz";
            lblihbelave.ForeColor = Color.Red;
        }
    ...
    protected void btnihbredakte_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                lblihbredakte.Text = "Sosial və humanitar sahə seçməmisiniz";
            }
            if (txtihbredakte.Text == "")
            {
                lblihbredakte.Text = "Sosial və humanitar sahə adı boşluq ola bilməz";
            }
            if (drrayon != null)
            {
    ...
    protected void btnihbsil_Click(object sender, EventArgs e)
    {
        if (ddlihbsil.SelectedValue != "0" && ddlihbraysil.SelectedValue != "0")
        {
            ...
            lblihbsil.Text = "Sosial və humanitar sahə silindi";
        ...
        {
            lblihbsil.Text = "Sosial və humanitar sahə seçilməyib";
    ...
    protected void btnyerinideyiw_Click(object sender, EventArgs e)
    {
        ...
        {
            obyekt = klas.GetDataRow("Select * from List_classifications_SHablon Where ListID=10 and ClassifID='" + ddlobject.SelectedValue + "'");
            obyekt1 = obyekt["SortA_Z"].ToString();
            int objekt = int.Parse(obyekt1.ToString());
            //Response.Write(obyekt1);
            if (ddlyeri.SelectedValue != "0")
            {
                classifid = klas.GetDataRow("Select * from List_classifications_SHablon Where ListID=10 and ClassifID='" + ddlyeri.SelectedValue + "'");
                classifid1 = classifid["SortA_Z"].ToString();
            }
            else
            {
            ...
            for (int i = 0; i < sorthamisi.Rows.Count; i++)
            {
                ...
                if ((secilmiwsortid >= int.Parse(sorthamisi.Rows[i]["SortA_Z"].ToString())) && (objekt < int.Parse(sorthamisi.Rows[i]["SortA_Z"].ToString())))
                {
                    ...
                    SqlConnection baglanti1 = klas.baglan();
                    SqlCommand cmd12 = new SqlCommand("Update List_classifications_SHablon Set SortA_Z=@SortA_Z Where ListID=10 and SortA_Z='" + objekt.ToString() + "' and ClassifID='" + ddlobject.SelectedValue + "'", baglanti1);
        ...
        else
        {
            ...
            {
                lblyerdeyiw.Text = "Sosial və humanitar sahə seçilməyib";
                lblyerdeyiw.ForeColor = Color.Red;
            }
