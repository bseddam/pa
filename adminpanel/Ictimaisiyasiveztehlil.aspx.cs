...
public partial class adminpanel_Ictimaisiyasiveztehlil : System.Web.UI.Page
{
    ...
    void sifirinci()
    {
        ...
        {
            siralama = "lll.UserID,lll.RegionID,sort1";
            lblsira.Text = "1";
        }
        else
        {
            siralama = "sort1,lll.UserID,lll.RegionID";
            lblsira.Text = "2";
        }
...
//        lblcemmeq.Text = klas.getdatacell(@"select COUNT(*) as meqcem from (SELECT  religious_politicalID,ReligiousOther FROM  
//Conn_religious_political  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
...
List_classification_Regions  as r inner join business_trip as bis ON r.RegionsID = bis.RegionID where AssortinID=4 " +
 tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + ") as cc");
        lblfmetn.Text = klas.getdatacell(@"select COUNT(*) as fmeqsay from ( SELECT distinct religious_politicalID,ReligiousOther FROM 
Conn_religious_political  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where  AssortinID=4 " +
        tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + ")as cc");



        DataTable region2 = klas.getdatatable(@"select  CONVERT(varchar(1000),meqsedumumi) as  meqsedumumi,meqseds,u.Name+' '+u.Sname  as Username,
lll.RegionID,lll.Offer,lll.Implementation
,u.UserID,lll.religious_politicalID,lll.ReligiousOther,SubReligiousOther,NoteText,
CONVERT(varchar(1000),ryns.rynsay)rynsay,r.Name,
case when b.sort Is null then 100000 else b.sort  end  as sort1
 from 

(select religious_politicalID,ReligiousOther,Sub_religious_politicalID,SubReligiousOther,NoteText,Offer,
Implementation,RegionID,UserID
  from business_trip bis inner join Conn_religious_political o on bis.BusinessTripID=o.BusinessTripID where AssortinID=4  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
 " " + userid + " ) as lll   inner join (SELECT COUNT(*) meqsedumumi,religious_politicalID,ReligiousOther FROM   Conn_religious_political  as r inner join "+
 "business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where AssortinID=4  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
 " " + userid + " group by religious_politicalID,ReligiousOther) " +
 "as mqsumumi on mqsumumi.religious_politicalID=lll.religious_politicalID and mqsumumi.ReligiousOther=lll.ReligiousOther " +


 "inner join (SELECT COUNT(*) meqseds,religious_politicalID,RegionID,UserID,ReligiousOther FROM  "+
 "Conn_religious_political  as r inner join  business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where AssortinID=4  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
 " " + userid + " group by religious_politicalID,RegionID,UserID,ReligiousOther) as mqs " +
 "on  mqs.religious_politicalID=lll.religious_politicalID and  mqs.ReligiousOther=lll.ReligiousOther and mqs.RegionID=lll.RegionID and mqs.UserID=lll.UserID " +
 "inner join ( SELECT COUNT(*) as rynsay,UserID,RegionID FROM business_trip as btr  where AssortinID=4  "
      ...
      " left join List_classifications as b on b.ClassifID=lll.religious_politicalID  "+
      "  order by " + siralama + "");
        ...
        if (rdrayon.Checked == true)
        {
            DataList1.DataSource = klas.tekrarlama1("Username", "rynsay", "Name", "UserID", "meqsedumumi", "ReligiousOther", "Sn", region2);
        }
        else
        {
            DataList1.DataSource = klas.tekrarlama01("Username", "ReligiousOther", "meqsedumumi", "rynsay", "Name", "UserID", "Sn", region2);
    ...
    void birinci()
    {
        ...
        if (PurposeID != null)
        {
            purp = "and religious_politicalID=" + PurposeID + " ";
        }
        else
        {
            purp = " ";
        ...
        DataTable region2 = klas.getdatatable(@"select BusinessTripID,RegionID,RegionCome,GoFromRegion,u.UserID,ReligiousOther,religious_politicalID,sort
,ClassifID,ClassifName,u.Name+' '+Sname as Username,r.Name   from (SELECT b.BusinessTripID, b.RegionID, b.RegionCome, b.GoFromRegion, b.Regions,
   b.UserID, c.ReligiousOther, CONVERT(varchar(1000), c.religious_politicalID) as religious_politicalID  FROM  business_trip as b INNER JOIN Conn_religious_political as c ON b.BusinessTripID = c.BusinessTripID where AssortinID=4
...
" ON ccc.RegionID = r.RegionsID  left join List_classifications as p on p.ClassifID=ccc.religious_politicalID " +
" order by " + sirala + " ");
