...
public partial class adminpanel_Gordukleriiw : System.Web.UI.Page
{
...
List_classification_Regions  as r inner join business_trip as bis ON r.RegionsID = bis.RegionID where AssortinID=4 " +
        ...
        DataTable region2 = klas.getdatatable(@"select u.Name+' '+Sname  as Username,mqs.BusinessTripID ,mqs.ClassifName as Other,WorkView,ResultView,
...
(SELECT COUNT(*) meqseds,PurposeId,RegionID,bis.UserID,ClassifName,WorkView,ResultView,bis.BusinessTripID  FROM   Conn_PurposeOfTrip  as r inner join 
business_trip as bis ON r.BusinessTripID = bis.BusinessTripID inner join List_classifications as lc on lc.ClassifID=r.PurposeId where AssortinID=4  "
            + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid +
            "  group by PurposeId,RegionID,bis.UserID,ClassifName,WorkView,ResultView,bis.BusinessTripID ) as mqs " +
