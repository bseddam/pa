...
public partial class adminpanel_Meqsedezamsay : System.Web.UI.Page
{
    ...
    void sifirinci()
    {
        ...
        lblfmetn.Text = klas.getdatacell(@"select COUNT(*) as fmeqsay from ( SELECT distinct PurposeId FROM 
Conn_PurposeOfTrip  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID
where  AssortinID=4 and PurposeId<>0 " +
...
//         tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + ") as cc");
        ...
        //Conn_PurposeOfTrip  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
        //where AssortinID=4 and PurposeId<>0 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + " )as cc");

//        DataTable region2 = klas.getdatatable(@"select u.Name+' '+Sname  as Username,mqs.ClassifName as Other,
//mqs.RegionID,u.UserID,mqs.PurposeId,meqseds,CONVERT(varchar(1000), ryns.rynsay) as rynsay,r.Name,
//case when sort Is null then 100000 else sort  end  as sort,CONVERT(varchar(1000),meqsedumumi) as  meqsedumumi from  
//(SELECT COUNT(*) meqseds,PurposeId,RegionID,bis.UserID,ClassifName FROM   Conn_PurposeOfTrip  as r inner join 
//business_trip as bis ON r.BusinessTripID = bis.BusinessTripID inner join List_classifications as lc on lc.ClassifID=r.PurposeId where AssortinID=4  "
//            + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid +
//            "  group by PurposeId,RegionID,bis.UserID,ClassifName) as mqs " +

//           " inner join (SELECT COUNT(*) meqsedumumi,PurposeId FROM   Conn_PurposeOfTrip  as r inner join  " +
//           " business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where AssortinID=4  "
//            + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid +
//            "  group by PurposeId) as mqsumumi on mqsumumi.PurposeId=mqs.PurposeId   " +
...
//" ON mqs.RegionID = r.RegionsID left join List_classifications as p on p.ClassifID=mqs.PurposeId " +
//" order by " + siralama + "");

        DataTable region2 = klas.getdatatable(@"select mqs.ClassifName as Other,mqs.PurposeId,meqseds,
case when sort Is null then 100000 else sort  end  as sort from  
(SELECT COUNT(*) meqseds,PurposeId,ClassifName FROM   Conn_PurposeOfTrip  as r inner join 
business_trip as bis ON r.BusinessTripID = bis.BusinessTripID inner join List_classifications as lc on lc.ClassifID=r.PurposeId 
where AssortinID=4 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid +
"  group by PurposeId,ClassifName) as mqs left join List_classifications as p on p.ClassifID=mqs.PurposeId  " );
...
,ClassifID,ClassifName,u.Name+' '+Sname as Username,r.Name   from (SELECT b.BusinessTripID, b.RegionID, b.RegionCome, b.GoFromRegion, b.Regions,c.WorkView,c.ResultView, 
   b.UserID, c.Other, CONVERT(varchar(1000), c.PurposeID) as PurposeID  FROM  business_trip as b INNER JOIN Conn_PurposeOfTrip as c ON b.BusinessTripID = c.BusinessTripID where AssortinID=4 and c.PurposeID<>0
" + rayon + "   " + tarixs22 + " " + tarixs11 + " " + user1 + " " + purp + " " + 
