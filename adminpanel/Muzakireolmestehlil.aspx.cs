...
public partial class adminpanel_Muzolmamiwmes : System.Web.UI.Page
{
    ...
    void sifirinci()
    {
        ...
        lblferqlimescem.Text = klas.getdatacell(@"select COUNT( distinct lll.ClassifID ) from (
select distinct btr.UserID,c.ClassifID,c.ClassifName,btr.RegionID,sort from List_classifications c,Business_trip btr 
where c.ListID=18 and c.ClassifID  not in (Select distinct p1.DiscussedID from business_trip bis inner join " +
...
" where bis.AssortinID=4  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
        " " + userid + "  and p1.BaxilibYesNo='true' and p1.OlunubYesNo='true' ) and btr.AssortinID=4 " + tarixs22 + " " + " " + tarixs11 + "  " + rayon1 + "  " + " " + userid1 + " "
  + ") as lll inner join " + "(SELECT COUNT(*) as rynsay,UserID,RegionID FROM business_trip as btr  where btr.AssortinID=4  "
...
,u.Name+' '+Sname as username ,r.Name from (
select distinct btr.UserID,c.ClassifID,c.ClassifName,btr.RegionID,sort from List_classifications c,Business_trip btr 
where c.ListID=18 and c.ClassifID  not in (Select distinct p1.DiscussedID from business_trip bis inner join " +
"Conn_participation_measure p1 on p1.BusinessTripID=bis.BusinessTripID and bis.RegionID=btr.RegionID and bis.UserID=btr.UserID " +
" where bis.AssortinID=4  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
 ...
 "left join List_classification_Regions as r ON lll.RegionID = r.RegionsID order by lll.UserID,lll.RegionID,lll.sort");
        ...
        DataList1.DataSource = klas.tekrarlamaxdd("Username", "rynsay", "Name", "UserID", "Sn", region2);
        DataList1.DataBind();
