...
public partial class adminpanel_Isetmediyitedbirtehlil : System.Web.UI.Page
{
    ...
    void sifirinci()
    {
        ...
        lblferqlicem.Text = klas.getdatacell(@"select count(distinct lll.ClassifID) 
from (
select distinct btr.UserID,c.ClassifID,c.ClassifName,btr.RegionID,sort from List_classifications c,Business_trip btr 
where c.ListID=9 and c.ClassifID  not in (
Select distinct p1.ParticipationID from business_trip bis 
inner join Conn_participation_measure p1 on p1.BusinessTripID=bis.BusinessTripID 
and bis.RegionID=btr.RegionID and bis.UserID=btr.UserID where p1.BaxilibYesNo='true' and p1.OlunubYesNo='true' and bis.AssortinID=4 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
        " " + userid + " union select distinct case when k.CityID=1 then 524 when k.CityID=2 then 3 end as ParticipationID from " +
" Conn_participation_measure as p1 " +
" inner join business_trip as bis on bis.BusinessTripID=p1.BusinessTripID " +
" inner join List_classification_Regions as k on k.RegionsID=bis.RegionID and bis.RegionID=btr.RegionID and bis.AssortinID=4 " +
" and p1.BaxilibYesNo='true' and p1.OlunubYesNo='true' " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
        " " + userid + ") and btr.AssortinID=4 ) as lll inner join (SELECT COUNT(*) as rynsay,UserID,RegionID FROM business_trip as bis  where  bis.AssortinID=4 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
        " " + userid + " group by UserID,RegionID )as ryns on ryns.RegionID=lll.RegionID and ryns.UserID=lll.UserID  " +
...
" and p1.BaxilibYesNo='true' and p1.OlunubYesNo='true' " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
 ...
 " inner join Users as u on u.UserID=lll.UserID  " +
 " left join List_classification_Regions as r ON lll.RegionID = r.RegionsID " +
 " order by lll.UserID,lll.RegionID,lll.sort");

//        DataTable region2 = klas.getdatatable(@"select lll.UserID,lll.ClassifID,lll.ClassifName,lll.RegionID,sort,CONVERT(varchar(1000), ryns.rynsay) as rynsay
//,u.Name+' '+Sname as username ,r.Name
//from (
//select distinct btr.UserID,c.ClassifID,c.ClassifName,btr.RegionID,sort from List_classifications c,Business_trip btr 
//where c.ListID=9 and c.ClassifID  not in (Select distinct p1.ParticipationID from business_trip bis inner join " +
//"Conn_participation_measure p1 on p1.BusinessTripID=bis.BusinessTripID and bis.RegionID=btr.RegionID and bis.UserID=btr.UserID " +
//" where p1.BaxilibYesNo='true' and p1.OlunubYesNo='true'  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
//        " " + userid + " ) and 1=1 " + tarixs22 + " " + " " + tarixs11 + "  " + rayon1 + "  " + " " + userid1 + " "
//  + ") as lll inner join " + "(SELECT COUNT(*) as rynsay,UserID,RegionID FROM business_trip as btr  where 1=1  "
