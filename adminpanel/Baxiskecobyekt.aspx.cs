...
public partial class adminpanel_Baxiskecobyekt : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        LinkButton tb = cp.FindControl("Baxiskecobyekt") as LinkButton;
    ...
    void ihbredakte1()
    {
        DataTable dtilceler = klas.getdatatable("Select * from List_SubClassifictions Where ListID=" + ddlicra.SelectedValue + " order by [sort]");
    ...
    {
        DataTable dtiller2 = klas.getdatatable("Select ClassifID,ClassifName,ListID from List_classifications where ListID=12 order by sort");
    ...
    void ihbredakte()
    {
        DataTable dtilceler = klas.getdatatable("Select * from List_SubClassifictions Where ListID=" + ddlihbrayonredakte.SelectedValue + " order by [sort]");
    ...
    void ihbsil()
    {
        DataTable dtilceler = klas.getdatatable("Select * from List_SubClassifictions Where ListID=" + ddlihbraysil.SelectedValue + " order by [sort]");
    ...
    {
        DataRow drrayon = klas.GetDataRow("Select * from List_classifications Where ListID=12 and ClassifName=N'" + txtmeqsedredakte.Text + "'");

        if (ddlmeqsedredakte.SelectedValue != "0" && txtmeqsedredakte.Text != "" && drrayon==null)
        {
            SqlConnection baglanti = klas.baglan();
            SqlCommand cmd = new SqlCommand("Update List_classifications Set ClassifName=@ClassifName Where ListID=12 and ClassifID=" + ddlmeqsedredakte.SelectedValue, baglanti);
            ...
            ddlmeqsedredakte.Items.Insert(0, new ListItem("Seçin", "0"));
            txtmeqsedredakte.Text = "";
            Response.Redirect("Baxiskecobyekt.aspx");
        }
        else
        {
            if (ddlmeqsedredakte.SelectedValue == "0")
            {
                lblmeqsedredakte.Text = "Baxış keçirilmiş istismarda olan obyekt seçməmisiniz";
            }
            if (txtmeqsedredakte.Text == "")
            {
                lblmeqsedredakte.Text = "Baxış keçirilmiş istismarda olan obyekt boşluq ola bilməz";
            }
            if (drrayon != null) {
    ...
    protected void btnmeqsedelave_Click(object sender, EventArgs e)
    {
        ...
        {
            DataRow drrayon = klas.GetDataRow("Select * from List_classifications Where ListID=12 and ClassifName=N'" + txtmeqsedelave.Text + "'");
            if (drrayon == null && ddlmeqsedsort.SelectedValue != "-1")
            {
                ...
                cmd.ExecuteNonQuery();
                lblmeqsedelave.Text = "Baxış keçirilmiş istismarda olan obyekt əlavə edildi";
        ...
        {
            lblmeqsedelave.Text = "Baxış keçirilmiş istismarda olan obyekt adı boşluq ola bilməz";
            lblmeqsedelave.ForeColor = Color.Red;
        }
    }
    protected void btnmeqsedsil_Click(object sender, EventArgs e)
    {
        if (ddlmeqsedsil.SelectedValue != "0")
        {
            DataRow classifid;
            string classifid1;
            classifid = klas.GetDataRow("Select * from List_classifications Where ListID=12 and ClassifID='" + ddlmeqsedsil.SelectedValue + "'");
            classifid1 = classifid["sort"].ToString();
            int secilmiwsortid = int.Parse(classifid1.ToString());
            ...
            klas.cmd("Delete from List_classifications  Where ListID=12 and ClassifID=" + ddlmeqsedsil.SelectedValue);
            lblmeqsedsil.Text = "Baxış keçirilmiş istismarda olan obyekt silindi";
            lblmeqsedsil.ForeColor = Color.Green;
            Response.Redirect("Baxiskecobyekt.aspx");
        }
        else
        {
            lblmeqsedsil.Text = "Baxış keçirilmiş istismarda olan obyekt seçilməyib";
    ...
    protected void btnihbelave_Click(object sender, EventArgs e)
    {
        ...
        {
            DataRow drrayon = klas.GetDataRow("Select * from List_SubClassifictions Where ListID='" + ddlihbrayonelave.SelectedValue + "' and  ClassificationName=N'" + txtihbelave.Text + "' order by [ClassificationName]");
            if (drrayon == null && ddlmovqe.SelectedValue != "-1")
            {
                if (ddlmovqe.SelectedValue != "0")
                {
                    classifid = klas.GetDataRow("Select * from List_SubClassifictions Where ListID='" + ddlihbrayonelave.SelectedValue + "' and ClassifID='" + ddlmovqe.SelectedValue + "'");
                    classifid1 = classifid["sort"].ToString();
                }
                else
                {
                ...
                cmd.Parameters.Add("sort", int.Parse(classifid1.ToString()) + 1);
                cmd.ExecuteNonQuery();
                lblihbelave.Text = "Obyekt əlavə edildi";
            ...
            else
            {
                if (drrayon != null)
                {
                    lblihbelave.Text = "Elavə etmək istədiyiniz obyekt artıq bazada var";
        ...
        {
            lblihbelave.Text = "Obyekt boşluq ola bilməz";
            lblihbelave.ForeColor = Color.Red;
        }
    ...
    {
        DataRow drrayon = klas.GetDataRow("Select * from List_SubClassifictions Where ClassifID='" + ddlihbredakte.SelectedValue+"' and ClassificationName=N'" + txtihbredakte.Text + "'");

        if (ddlihbredakte.SelectedValue != "0" && txtihbredakte.Text != "" && drrayon==null)
        {
            SqlConnection baglanti = klas.baglan();
            SqlCommand cmd = new SqlCommand("Update List_SubClassifictions Set ClassificationName=@ClassificationName Where ClassifID=" + ddlihbredakte.SelectedValue, baglanti);
        ...
        else
        {
            ...
            {
                lblihbredakte.Text = "Obyekt seçməmisiniz";
            }
            if (txtihbredakte.Text == "")
            {
                lblihbredakte.Text = "Obyekt adı boşluq ola bilməz";
            }
            if (drrayon != null) {
    ...
    protected void btnihbsil_Click(object sender, EventArgs e)
    {
        if (ddlihbsil.SelectedValue != "0" && ddlihbraysil.SelectedValue != "0")
        {
            DataRow classifid;
            string classifid1;
            classifid = klas.GetDataRow("Select * from List_SubClassifictions Where ListID='" + ddlihbraysil.SelectedValue + "' and ClassifID='" + ddlihbsil.SelectedValue + "'");
            classifid1 = classifid["sort"].ToString();
            int secilmiwsortid = int.Parse(classifid1.ToString());

            DataTable sorthamisi = klas.getdatatable("Select * from List_SubClassifictions Where ListID=" + ddlihbraysil.SelectedValue);
            for (int i = 0; i < sorthamisi.Rows.Count; i++)
            {
                if (secilmiwsortid < int.Parse(sorthamisi.Rows[i]["sort"].ToString()))
                {
                    ...
                    int classif = int.Parse(sorthamisi.Rows[i]["ClassifID"].ToString());
                    SqlConnection baglanti = klas.baglan();
                    SqlCommand cmd1 = new SqlCommand("Update List_SubClassifictions Set sort=@sort Where ListID='" + ddlihbraysil.SelectedValue + "' and sort='" + a.ToString() + "' and ClassifID='" + classif.ToString() + "'", baglanti);
                    cmd1.Parameters.Add("sort", a1);
                    cmd1.ExecuteNonQuery();
                }
    ...
    protected void btnyerinideyiw_Click(object sender, EventArgs e)
    {
        ...
        {
            obyekt = klas.GetDataRow("Select * from List_classifications Where ListID=12 and ClassifID='" + ddlobject.SelectedValue + "'");
            obyekt1 = obyekt["sort"].ToString();
            int objekt = int.Parse(obyekt1.ToString());
            //Response.Write(obyekt1);
            if (ddlyeri.SelectedValue != "0")
            {
                classifid = klas.GetDataRow("Select * from List_classifications Where ListID=12 and ClassifID='" + ddlyeri.SelectedValue + "'");
                classifid1 = classifid["sort"].ToString();
            }
            else
            {
            ...
            for (int i = 0; i < sorthamisi.Rows.Count; i++)
            {
                ...
                if ((secilmiwsortid >= int.Parse(sorthamisi.Rows[i]["sort"].ToString())) && (objekt < int.Parse(sorthamisi.Rows[i]["sort"].ToString())))
                {
                    ...
                    SqlConnection baglanti1 = klas.baglan();
                    SqlCommand cmd12 = new SqlCommand("Update List_classifications Set sort=@sort Where ListID=12 and sort='" + objekt.ToString() + "' and ClassifID='" + ddlobject.SelectedValue + "'", baglanti1);
        ...
        else
        {
            ...
            {
                lblyerdeyiw.Text = "Obyekt seçilməyib";
                lblyerdeyiw.ForeColor = Color.Red;
            }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            obyekt = klas.GetDataRow("Select * from List_SubClassifictions Where ListID=" + ddlicra.SelectedValue + " and ClassifID='" + ddlvezife.SelectedValue + "'");
            obyekt1 = obyekt["sort"].ToString();
            int objekt = int.Parse(obyekt1.ToString());
            //Response.Write(obyekt1);
            if (ddlyer.SelectedValue != "0")
            {
                classifid = klas.GetDataRow("Select * from List_SubClassifictions Where ListID=" + ddlicra.SelectedValue + " and ClassifID='" + ddlyer.SelectedValue + "'");
                classifid1 = classifid["sort"].ToString();
            }
            else
            {
                classifid = klas.GetDataRow("Select MIN(sort) as sort from List_SubClassifictions Where ListID=" + ddlicra.SelectedValue + "");
                //int.Parse(classifid.ToString()) - 1;
                classifid1 = (int.Parse(classifid["sort"].ToString()) - 1).ToString();
            }
            //Response.Write(classifid1);
            int secilmiwsortid = int.Parse(classifid1.ToString());
            DataTable sorthamisi = klas.getdatatable("Select * from List_SubClassifictions Where ListID=" + ddlicra.SelectedValue + "");
            for (int i = 0; i < sorthamisi.Rows.Count; i++)
            {
                ...
                if ((secilmiwsortid >= int.Parse(sorthamisi.Rows[i]["sort"].ToString())) && (objekt < int.Parse(sorthamisi.Rows[i]["sort"].ToString())))
                {
                    ...
                    int classif = int.Parse(sorthamisi.Rows[i]["ClassifID"].ToString());
                    SqlConnection baglanti = klas.baglan();
                    SqlCommand cmd1 = new SqlCommand("Update List_SubClassifictions Set sort=@sort Where ListID=" + ddlicra.SelectedValue + " and sort='" + a.ToString() + "' and ClassifID='" + classif.ToString() + "'", baglanti);
                    cmd1.Parameters.Add("sort", a1);
                    cmd1.ExecuteNonQuery();
                    //Response.Write("obyekt="+objekt.ToString());
                    //Response.Write("yer=" + classifid1.ToString());
                    SqlConnection baglanti1 = klas.baglan();
                    SqlCommand cmd12 = new SqlCommand("Update List_SubClassifictions Set sort=@sort Where ListID=" + ddlicra.SelectedValue + " and sort='" + objekt.ToString() + "' and ClassifID='" + ddlvezife.SelectedValue + "'", baglanti1);
                    cmd12.Parameters.Add("sort", classifid1);
                    cmd12.ExecuteNonQuery();
