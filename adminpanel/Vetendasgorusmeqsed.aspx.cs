...
public partial class adminpanel_Vetendasgorusmeqsed : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        LinkButton tb = cp.FindControl("Vetendasgorusmeqsed") as LinkButton;
    ...
    {
        DataRow drrayon = klas.GetDataRow("Select * from List_classifications Where ListID=13 and ClassifName=N'" + txtmeqsedredakte.Text + "'");

        if (ddlmeqsedredakte.SelectedValue != "0" && txtmeqsedredakte.Text != "" && drrayon==null)
        {
            SqlConnection baglanti = klas.baglan();
            SqlCommand cmd = new SqlCommand("Update List_classifications Set ClassifName=@ClassifName Where ListID=13 and ClassifID=" + ddlmeqsedredakte.SelectedValue, baglanti);
            ...
            Response.Redirect("Vetendasgorusmeqsed.aspx");
        }
        else
        {
            if (ddlmeqsedredakte.SelectedValue == "0")
            {
                lblmeqsedredakte.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədini seçməmisiniz";
            }
            if (txtmeqsedredakte.Text == "")
            {
                lblmeqsedredakte.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədi boşluq ola bilməz";
            }
            if (drrayon != null) {
    ...
    protected void btnmeqsedelave_Click(object sender, EventArgs e)
    {
        ...
        {
            DataRow drrayon = klas.GetDataRow("Select * from List_classifications Where ListID=13 and ClassifName=N'" + txtmeqsedelave.Text + "'");
            if (drrayon == null && ddlmeqsedsort.SelectedValue != "-1")
            {
                ...
                cmd.ExecuteNonQuery();
                lblmeqsedelave.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədi əlavə edildi";
        ...
        {
            lblmeqsedelave.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədi boşluq ola bilməz";
            lblmeqsedelave.ForeColor = Color.Red;
        }
    }
    protected void btnmeqsedsil_Click(object sender, EventArgs e)
    {
        if (ddlmeqsedsil.SelectedValue != "0")
        {
            DataRow classifid;
            string classifid1;
            classifid = klas.GetDataRow("Select * from List_classifications Where ListID=13 and ClassifID='" + ddlmeqsedsil.SelectedValue + "'");
            classifid1 = classifid["sort"].ToString();
            int secilmiwsortid = int.Parse(classifid1.ToString());
            ...
            klas.cmd("Delete from List_classifications  Where ListID=13 and ClassifID=" + ddlmeqsedsil.SelectedValue);
            lblmeqsedsil.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədi silindi";
            lblmeqsedsil.ForeColor = Color.Green;
            Response.Redirect("Vetendasgorusmeqsed.aspx");
        }
        else
        {
            lblmeqsedsil.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədi seçilməyib";
    ...
    protected void btnyerinideyiw_Click(object sender, EventArgs e)
    {
        ...
        {
            obyekt = klas.GetDataRow("Select * from List_classifications Where ListID=13 and ClassifID='" + ddlobject.SelectedValue + "'");
            obyekt1 = obyekt["sort"].ToString();
            int objekt = int.Parse(obyekt1.ToString());
            //Response.Write(obyekt1);
            if (ddlyeri.SelectedValue != "0")
            {
                classifid = klas.GetDataRow("Select * from List_classifications Where ListID=13 and ClassifID='" + ddlyeri.SelectedValue + "'");
                classifid1 = classifid["sort"].ToString();
            }
            else
            {
            ...
            for (int i = 0; i < sorthamisi.Rows.Count; i++)
            {
                ...
                if ((secilmiwsortid >= int.Parse(sorthamisi.Rows[i]["sort"].ToString())) && (objekt < int.Parse(sorthamisi.Rows[i]["sort"].ToString())))
                {
                    ...
                    SqlConnection baglanti1 = klas.baglan();
                    SqlCommand cmd12 = new SqlCommand("Update List_classifications Set sort=@sort Where ListID=13 and sort='" + objekt.ToString() + "' and ClassifID='" + ddlobject.SelectedValue + "'", baglanti1);
        ...
        else
        {
            ...
            {
                lblyerdeyiw.Text = "Vətəndaşlarla keçirilmiş görüşün məqsədi seçilməyib";
                lblyerdeyiw.ForeColor = Color.Red;
            }
