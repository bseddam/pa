...
public partial class adminpanel_IHBtehlil : System.Web.UI.Page
{
...
List_classification_Regions  as r inner join business_trip as bis ON r.RegionsID = bis.RegionID where AssortinID=4 " +
       tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + ") as cc");
        lblfmetn.Text = klas.getdatacell(@"select COUNT(*) yasmensayi from Administrative_Department where ForDelete=1");
        lblolyasmen.Text = klas.getdatacell(@"SELECT COUNT(*) as oldsay FROM   Conn_delegate_IHB  as cl  
inner join  business_trip as bis ON cl.BusinessTripID = bis.BusinessTripID 
left join Administrative_Department as sr on sr.DepartmentID=cl.DelegateID and bis.RegionID=sr.RegionID 
 where AssortinID=4 and IshtirakYesNo='true' and DelegateID<>0 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + "");

        lblolferqli.Text = klas.getdatacell(@"select COUNT(*) from (SELECT distinct bis.RegionID,DelegateID,bis.UserID FROM   Conn_delegate_IHB  as cl  
inner join  business_trip as bis ON cl.BusinessTripID = bis.BusinessTripID 
left join Administrative_Department as sr on sr.DepartmentID=cl.DelegateID and bis.RegionID=sr.RegionID 
 where AssortinID=4 and IshtirakYesNo='true' and DelegateID<>0 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + " ) as c ");
        ...
        DataTable region2 = klas.getdatatable(@"select u.Name+' '+Sname  as Username,CONVERT(varchar(1000),cemimensayi) cemimensayi,mqs.DelegateOther as DelegateOther,CONVERT(varchar(1000), oldsayi) oldsayi,
CONVERT(varchar(1000),CONVERT(decimal(5,1),(Round((CONVERT(decimal,meqseds)/CONVERT(decimal,cemimensayi))*100,1)))) + '%' as ehatefaiz,
mqs.RegionID,u.UserID,mqs.DelegateID,meqseds,CONVERT(varchar(1000), ryns.rynsay) as rynsay,r.Name from 
(SELECT COUNT(*) meqseds,DelegateID,bis.RegionID,bis.UserID,DelegateOther FROM   Conn_delegate_IHB  as r inner join 
business_trip as bis ON r.BusinessTripID = bis.BusinessTripID inner join Administrative_Department as lc on lc.DepartmentID=r.DelegateID 
where AssortinID=4 and IshtirakYesNo='true' and lc.ForDelete=1 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +" " + userid +
" group by DelegateID,bis.RegionID,bis.UserID,DelegateOther) as mqs " +
" inner join (select COUNT(*) cemimensayi,RegionID from Administrative_Department as sr where ForDelete=1 " +
" group by RegionID) as mn on mn.RegionID=mqs.RegionID " +
" inner join (select COUNT(*) as oldsayi,UserID,RegionID from ( " +
" select distinct bis.RegionID,DepartmentName,bis.UserID from Conn_delegate_IHB as r " +
" inner join Administrative_Department as lc on lc.DepartmentID=r.DelegateID inner join " +
...
" " + userid + 
" ) as kkk group by UserID,RegionID) as oldsayi  " +
...
" left join List_classification_Regions as r ON mqs.RegionID = r.RegionsID  " +
 " order by mqs.UserID,mqs.RegionID,DelegateOther");

//        DataTable region2 = klas.getdatatable(@"select u.Name+' '+Sname  as Username,CONVERT(varchar(1000),cemimensayi) cemimensayi,
//CONVERT(varchar(1000),CONVERT(decimal(5,1),(Round((CONVERT(decimal,meqseds)/CONVERT(decimal,cemimensayi))*100,1)))) + '%' as ehatefaiz,
//mqs.RegionID,mqs.DelegateOther,u.UserID,mqs.DelegateID,meqseds,CONVERT(varchar(1000), ryns.rynsay) as rynsay,r.Name from 
//(SELECT COUNT(*) meqseds,DelegateID,RegionID,UserID,DelegateOther FROM   Conn_delegate_IHB  as r inner join 
//business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where AssortinID=4 and IshtirakYesNo='true' " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
//        " " + userid + "  group by DelegateID,RegionID,UserID,DelegateOther) as mqs " +
//        "inner join ( SELECT COUNT(*) as rynsay,UserID,RegionID FROM business_trip as btr  where AssortinID=4 " + tarixs22 + " " + " " + tarixs11 + "  " + rayon1 + "  " +
//        " " + userid1 + " " +
//        "group by UserID,RegionID )as ryns on ryns.RegionID=mqs.RegionID and ryns.UserID=mqs.UserID " +
//        "inner join (select COUNT(*) cemimensayi,RegionID from Administrative_Department as sr where ForDelete=1 " +
//"group by RegionID) as mn on mn.RegionID=mqs.RegionID " +
...
// "order by mqs.UserID,mqs.RegionID,DelegateOther");
    ...
    void birinci()
    {
        ...
        {
            LivingID = "and DelegateID=" + LivingID;
        }
        pnlesasseh.Visible = false;
        pnlbirinci.Visible = true;
   ...
   bis.UserID, c.DelegateOther, c.DelegateID FROM  business_trip as bis INNER JOIN Conn_delegate_IHB as c ON bis.BusinessTripID = c.BusinessTripID 
