...
public partial class adminpanel_Baxilmisarasmes : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        LinkButton tb = cp.FindControl("Baxilmisarasmes") as LinkButton;
    ...
    {
        DataRow drrayon = klas.GetDataRow("Select * from List_classifications Where ListID=10 and ClassifName=N'" + txtmeqsedredakte.Text + "'");

        if (ddlmeqsedredakte.SelectedValue != "0" && txtmeqsedredakte.Text != "" && drrayon==null)
        {
            SqlConnection baglanti = klas.baglan();
            SqlCommand cmd = new SqlCommand("Update List_classifications Set ClassifName=@ClassifName Where ListID=10 and ClassifID=" + ddlmeqsedredakte.SelectedValue, baglanti);
            ...
            Response.Redirect("Baxilmisarasmes.aspx");
        }
        else
        {
            if (ddlmeqsedredakte.SelectedValue == "0")
            {
                lblmeqsedredakte.Text = "Baxılmış, araşdırılmış məsələ seçməmisiniz";
            }
            if (txtmeqsedredakte.Text == "")
            {
                lblmeqsedredakte.Text = "Baxılmış, araşdırılmış məsələ boşluq ola bilməz";
            }
            if (drrayon != null) {
    ...
    protected void btnmeqsedelave_Click(object sender, EventArgs e)
    {
        ...
        {
            DataRow drrayon = klas.GetDataRow("Select * from List_classifications Where ListID=10 and ClassifName=N'" + txtmeqsedelave.Text + "'");
        ...
        {
            lblmeqsedelave.Text = "Baxılmış, araşdırılmış məsələ adı boşluq ola bilməz";
            lblmeqsedelave.ForeColor = Color.Red;
        }
    }
    protected void btnmeqsedsil_Click(object sender, EventArgs e)
    {
        if (ddlmeqsedsil.SelectedValue != "0")
        {
            DataRow classifid;
            string classifid1;
            classifid = klas.GetDataRow("Select * from List_classifications Where ListID=10 and ClassifID='" + ddlmeqsedsil.SelectedValue + "'");
            classifid1 = classifid["sort"].ToString();
            int secilmiwsortid = int.Parse(classifid1.ToString());
            ...
            klas.cmd("Delete from List_classifications  Where ListID=10 and ClassifID=" + ddlmeqsedsil.SelectedValue);
            lblmeqsedsil.Text = "Baxılmış, araşdırılmış məsələ silindi";
            lblmeqsedsil.ForeColor = Color.Red;
            Response.Redirect("Baxilmisarasmes.aspx");
        }
        else
        {
            lblmeqsedsil.Text = "Baxılmış, araşdırılmış məsələ seçilməyib";
            lblmeqsedsil.ForeColor = Color.Red;
        }
        meqsedsil();
        ddlmeqsedsil.Items.Insert(0, new ListItem("Seçin", "0"));
    ...
    protected void btnyerinideyiw_Click(object sender, EventArgs e)
    {
        ...
        {
            obyekt = klas.GetDataRow("Select * from List_classifications Where ListID=10 and ClassifID='" + ddlobject.SelectedValue + "'");
            obyekt1 = obyekt["sort"].ToString();
            int objekt = int.Parse(obyekt1.ToString());
            //Response.Write(obyekt1);
            if (ddlyeri.SelectedValue != "0")
            {
                classifid = klas.GetDataRow("Select * from List_classifications Where ListID=10 and ClassifID='" + ddlyeri.SelectedValue + "'");
                classifid1 = classifid["sort"].ToString();
            }
            else
            {
            ...
            for (int i = 0; i < sorthamisi.Rows.Count; i++)
            {
                if ((secilmiwsortid >= int.Parse(sorthamisi.Rows[i]["sort"].ToString())) && (objekt < int.Parse(sorthamisi.Rows[i]["sort"].ToString())))
                {
                    ...
                    SqlConnection baglanti1 = klas.baglan();
                    SqlCommand cmd12 = new SqlCommand("Update List_classifications Set sort=@sort Where ListID=10 and sort='" + objekt.ToString() + "' and ClassifID='" + ddlobject.SelectedValue + "'", baglanti1);
        ...
        else
        {
            ...
            {
                lblyerdeyiw.Text = "Baxılmış, araşdırılmış məsələ seçilməyib";
                lblyerdeyiw.ForeColor = Color.Red;
            }
        }
    }
