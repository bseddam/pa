...
public partial class adminpanel_İhtovsiyetehlil : System.Web.UI.Page
{
    ...
    void sifirinci()
    {
        ...
        {
            if (ddlicrabarqeyd.SelectedValue == "1")
            {
                icrabarqeyd = " and CommandYesNo='true' ";
            }
            else if (ddlicrabarqeyd.SelectedValue == "2")
            {
                icrabarqeyd = " and CommandYesNo='false' ";
            }
            else {
                icrabarqeyd = "  ";
            }
        ...
        lblfmetn.Text = klas.getdatacell(@"select COUNT(*) as fmeqsay from ( SELECT distinct PurposeId,Other FROM 
Conn_PurposeOfTrip  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where  AssortinID=4 " +
...
List_classification_Regions  as r inner join business_trip as bis ON r.RegionsID = bis.RegionID where AssortinID=4 " +
         tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + ") as cc");
//        lblcemmeq.Text = klas.getdatacell(@"select COUNT(*) as meqcem from (SELECT  PurposeId,Other FROM  
//Conn_PurposeOfTrip  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
//where baxilib='true' and AssortinID=4 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + " )as cc");

        DataTable region2 = klas.getdatatable(@"select  u.Name+' '+u.Sname  as Username,CONVERT(varchar(1000),meqseds)meqseds,lll.BusinessTripID ,
CONVERT(varchar(1000),meqsedumumi)meqsedumumi,lll.RegionID
,u.UserID,lll.PurposeId,lll.Other,recommendedView,WorkView,CommandDate,CommandText,
CONVERT(varchar(1000),ryns.rynsay)rynsay,r.Name,
case when b.sort Is null then 100000 else b.sort  end  as sort1 from 
(select PurposeId,Other,WorkView,ResultView,recommendedView,RegionID,UserID,CommandDate,CommandText,bis.BusinessTripID 
  from business_trip bis inner join Conn_PurposeOfTrip o on bis.BusinessTripID=o.BusinessTripID
   where   AssortinID=4    " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + " " + icrabarqeyd + ") as lll " +
    " inner join (SELECT COUNT(*) meqseds,PurposeId,RegionID,UserID,Other FROM   Conn_PurposeOfTrip  as r inner join "+
"business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where  AssortinID=4    " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + " " + icrabarqeyd+
" group by PurposeId,RegionID,UserID,Other) as mqs on lll.PurposeId=mqs.PurposeId and lll.Other=mqs.Other and "+
"mqs.RegionID=lll.RegionID and mqs.UserID=lll.UserID   "+
          
" inner join (SELECT COUNT(*) meqsedumumi,PurposeId,Other FROM   Conn_PurposeOfTrip  as r inner join "+
           " business_trip as bis ON r.BusinessTripID = bis.BusinessTripID where  AssortinID=4    " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + " " + icrabarqeyd +
" group by PurposeId,Other) as mqsumumi on mqsumumi.PurposeId=mqs.PurposeId and mqsumumi.Other=mqs.Other" +

" inner join ( SELECT COUNT(*) as rynsay,UserID,RegionID FROM business_trip as bis  where AssortinID=4  " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid +
"  group by UserID,RegionID )as ryns on ryns.RegionID=lll.RegionID and ryns.UserID=lll.UserID   " +
     ...
     " left join List_classifications as b on b.ClassifID=lll.PurposeId   " +
" order by " + siralama + "");
    ...
    void birinci()
    {
        ...
        DataTable region2 = klas.getdatatable(@"select BusinessTripID,RegionID,RegionCome,GoFromRegion,WorkView,ResultView,u.UserID,Other,PurposeId,sort
,ClassifID,ClassifName,u.Name+' '+Sname as Username,r.Name   from (SELECT b.BusinessTripID, b.RegionID, b.RegionCome, b.GoFromRegion, b.Regions,c.WorkView,c.ResultView, 
   b.UserID, c.Other, CONVERT(varchar(1000), c.PurposeId) as PurposeId  FROM  business_trip as b INNER JOIN Conn_PurposeOfTrip as c ON b.BusinessTripID = c.BusinessTripID where AssortinID=4 
