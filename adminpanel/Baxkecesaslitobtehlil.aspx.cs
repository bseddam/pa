...
public partial class adminpanel_Baxkecesaslitobtehlil : System.Web.UI.Page
{
...
Conn_facilities  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
...
Conn_facilities  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
where AssortinID=4 " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + sahe + teyinat + " )as cc");
        lblaparilir.Text = klas.getdatacell(@"select COUNT(*) as meqcem from (SELECT  ConductedDate FROM  
Conn_facilities  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
where AssortinID=4 and ConductedDate is not null " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + sahe + teyinat + " )as cc");

        lbldayan.Text = klas.getdatacell(@"select COUNT(*) as meqcem from (SELECT  SuspendedDate FROM  
Conn_facilities  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
where AssortinID=4 and SuspendedDate is not null " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + sahe + teyinat + " )as cc");
        lblhazirdir.Text = klas.getdatacell(@"select COUNT(*) as meqcem from (SELECT  operationDate FROM  
Conn_facilities  as r inner join business_trip as bis ON r.BusinessTripID = bis.BusinessTripID 
where AssortinID=4 and operationDate is not null " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " + " " + userid + sahe + teyinat + " )as cc");
...
,lll.ObjectOther,lll.ObjectID,u.UserID,lll.SubObjectID,
case when hazirdir Is null then 0 else hazirdir  end  as hazirdir
,case when aparilir Is null then 0 else aparilir  end  as aparilir
,case when dayandirilib Is null then 0 else dayandirilib  end  as dayandirilib,
...
sum(case when r.ConductedDate IS Not null then 1 else 0 end) aparilir,
sum(case when r.SuspendedDate IS Not null then 1 else 0 end) dayandirilib,
sum(case when r.operationDate IS Not null then 1 else 0 end) hazirdir
 from business_trip bis inner join Conn_facilities r on bis.BusinessTripID=r.BusinessTripID  where AssortinID=4   " + tarixs2 + " " + " " + tarixs1 + "  " + rayon + "  " +
 ...
 " left join List_SubClassifictions as k on k.ClassifID=lll.SubObjectID  order by " + siralama + "");
    ...
    void birinci()
    {
        ...
        else if (Construction_works == "aparilir")
        {
            purp = "and ConductedDate is not null ";
        }
        else if (Construction_works == "dayandirilib")
        {
            purp = "and SuspendedDate is not null ";
        }
        else{
            //hazirdir
            purp = "and operationDate is not null ";
        }
  ...
  b.UserID,  case when c.ConductedDate IS not null then N'aparılır'
  when c.SuspendedDate IS not null then N'dayandırılıb'
  when c.operationDate IS not null then N'hazırdır' end as veziyyet,c.ConductedDate,c.SuspendedDate,c.operationDate,c.WorkView,c.ResultView,c.ObjectID,c.ObjectOther,SubObjectID,SubObjectOther,c.Living_Point,c.Living_PointID,
  case when lc.sort Is null then 100000 else lc.sort  end  as sort1,
case when k.sort Is null then 100000 else k.sort  end  as sort2
  FROM  business_trip as b INNER JOIN Conn_facilities as c ON b.BusinessTripID = c.BusinessTripID
